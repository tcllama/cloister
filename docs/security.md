...
| **Device passthrough grants direct kernel-device access** | `sandbox.devBinds` mounts device nodes with `--dev-bind`. Any process inside the sandbox can use those devices with the same host permissions as the launching user/group. Only bind devices you trust the sandboxed workload to access. |
