...
| **PulseAudio grants full audio including microphone** | When `audio.pulseaudio.enable` is set, Cloister forwards the host PulseAudio socket directly. PulseAudio has no per-client restriction mechanism, so the sandbox gets unrestricted playback and recording. |
...
| **Unfiltered PipeWire grants broad media access** | When `audio.pipewire.enable = true` and `audio.pipewire.filters.enable = false`, the sandbox can see the full PipeWire graph. Use `audio.pipewire.filters.*` to narrow access, or `audio.pipewire.pulseOnly = true` for audio-only sandboxes. |
...
| **`pipewire-pulse` is sandbox-wide, not per-process** | When `audio.pipewire.pulseCompat.enable` is set, Cloister starts `pipewire-pulse` inside the sandbox and exposes a local `pulse/native` socket. That does not widen host access beyond what the mounted PipeWire socket already allows, but every process inside the same sandbox can use the bridge. If PipeWire filters expose microphones, camera sources, volume control, or routing metadata, any sandbox process that can reach the local PulseAudio socket can exercise those same capabilities. |
| **`pipewire-pulse` without PipeWire filters inherits full PipeWire exposure** | `pulseCompat` is safer than direct host PulseAudio forwarding because it can sit on top of filtered PipeWire. But if `audio.pipewire.filters.enable = false`, PulseAudio-compatible apps talking to the bridge still inherit the full unrestricted PipeWire graph exposed to the sandbox. |
